...
class SubmissionBufferTest {
    ...
    @Test void emptyIsComplete() {
        // An empty buffer contributes no open delimiters — considered complete
        var b = buf();
        assertTrue(b.isComplete());
    }

    @Test void simplePrintIsComplete() {
        var b = buf();
        b.append("print 1;");
        assertTrue(b.isComplete());
    }

    @Test void unclosedParenIsIncomplete() {
        var b = buf();
        b.append("print (1 +");
        assertFalse(b.isComplete());
    }

    @Test void closedParenIsComplete() {
        var b = buf();
        b.append("print (1 + 2);");
        assertTrue(b.isComplete());
    }

    @Test void unclosedBraceIsIncomplete() {
        var b = buf();
        b.append("if (true) {");
        assertFalse(b.isComplete());
    }

    @Test void multiLineBlockIsIncomplete_ThenComplete() {
        var b = buf();
        b.append("if (true) {");
        assertFalse(b.isComplete());
        b.append("  print 1;");
    ...
    @Test void delimitersInsideStringIgnored() {
        var b = buf();
        b.append("print \"(\";"); // the ( inside the string should not count
        assertTrue(b.isComplete());
    }

    @Test void strayClosingParenTreatedAsComplete() {
