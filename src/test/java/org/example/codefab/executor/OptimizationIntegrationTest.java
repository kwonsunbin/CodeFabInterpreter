 ...
 * <ul>
 *   <li>정적 바인딩: {@code Variable.depth} / {@code Assign.depth} → {@code Environment.getAt/setAt} (O(1))</li>
...
class OptimizationIntegrationTest {
    ...
    // ════════════════════════════════════════════════════════════════════════
    // 1. 정적 바인딩 — depth로 정확한 스코프를 즉시 지정 (nearest 탐색이 아님)
    // ════════════════════════════════════════════════════════════════════════

    /**
     * 같은 이름 x가 global("global")과 outer 블록("outer")에 모두 존재한다.
     * 최내곽(빈) 블록에서 x를 읽되 depth를 직접 지정한다:
     *  depth 2 → global, depth 1 → outer, depth 0 → 최내곽(없음)
     */
    private String readShadowedXAtDepth(int depth) {
        ...
        var innerPrint = new Stmt.Print(varRead("x", depth));
    ...
    @Test void staticBinding_depth0_innermostHasNoBinding_doesNotWalkChain() {
        // depth 0 = 최내곽(빈 스코프). 체인을 거슬러 올랐다면 "outer"가 나오겠지만,
        // getAt(0)은 그 자리만 보므로 바인딩 없음 → nil. (정적 주소 접근 증명)
        assertEquals("nil", readShadowedXAtDepth(0));
    }

    @Test void staticBinding_assign_setAtWritesTargetedScope() {
        // 최내곽에서 x = "W"를 depth 2로 → global x에 기록.
    ...
    @Test void staticBinding_unresolvedDepth_fallsBackToDynamicLookup() {
        // depth -1 (미해석)이면 기존 동적 조회로 폴백
        var xTok = varTok("x");
